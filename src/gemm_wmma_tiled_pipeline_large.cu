...
__global__ __launch_bounds__(
    ...
    GEMM_MULTISTAGE_MIN_BLOCKS > 0
        ...
        : ((BM / WM) * (BN / WN) == 4
               ...
               : 2)) void gemm_wmma_tiled_pipeline_large(
...
static bool launch_large(const half *A, const half *B, half *C, int M, int N,
  ...
  if (DYNAMIC_BYTES) {
    ...
    if (cudaFuncSetAttribute(
            gemm_wmma_tiled_pipeline_large<UNIT_ALPHA_ZERO_BETA, BM, BN, BK,
                                           WM, WN, STAGES>,
...
static void solve_large(const half *A, const half *B, half *C, int M, int N,
  ...
  if (full_aligned) {
    ...
    if (M % 64 == 0 && N % 64 == 0 && K % 64 == 0 && K >= 192 &&
      ...
      launch_large<UNIT_ALPHA_ZERO_BETA, 64, 64, 64, 32, 32, 3>
          (A, B, C, M, N, K, alpha, beta);
      return;
    }
#endif
    if (launch_large<UNIT_ALPHA_ZERO_BETA, GEMM_BM, GEMM_BN, GEMM_BK,
