// Standalone successor to gemm_wmma_tiled_pipeline_multistage.cu.
// BK=64 amortizes mainloop control for small, complete output grids.
    ...
    GEMM_MULTISTAGE_MIN_BLOCKS > 0
        ...
        : ((BM / WM) * (BN / WN) == 4
               ? (BM == 64 && BN == 64 ? (BK == 64 ? 4 : 5)
                                       : (BM == 128 && BN == 128 ? 2 : 3))
               : 2)) void gemm_wmma_tiled_pipeline_mainloop(const half
                                                                *__restrict__ A,
                                                            const half
                                                                *__restrict__ B,
                                                            half
...
template <int BM, int BN, int BK, int WM, int WN, int STAGES>
static void launch_mainloop(const half *A, const half *B, half *C, int M, int N,
  ...
  gemm_wmma_tiled_pipeline_mainloop<BM, BN, BK, WM, WN, STAGES>
...
// Default tile selection uses 64x64, 64x128, or 128x128 output blocks and three
...
extern "C" void solve(const half *A, const half *B, half *C, int M, int N,
  ...
  if (full_aligned) {
    ...
    if (M % 128 == 0 && N % 128 == 0 && K % 32 == 0 && K >= 2048 &&
      ...
      launch_mainloop<128, 128, 32, 64, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
    if (M % 64 == 0 && N % 128 == 0 && K % 32 == 0 && K >= 512 &&
        (size_t(M) / 64) * (size_t(N) / 128) >= 256) {
      launch_mainloop<64, 128, 32, 32, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
    ...
    if (M % 64 == 0 && N % 64 == 0 && K % 64 == 0 && K >= 192 &&
      ...
      launch_mainloop<64, 64, 64, 32, 32, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
#endif
    launch_mainloop<GEMM_BM, GEMM_BN, GEMM_BK, GEMM_WM, GEMM_WN, GEMM_STAGES>(
