// Standalone successor to gemm_wmma_tiled_pipeline_mainloop.cu.
// A 96x128 block / 48x64 warp tile increases A/B reuse on populated grids.
...
__global__ __launch_bounds__(
    ...
    GEMM_MULTISTAGE_MIN_BLOCKS > 0
        ...
        : ((BM / WM) * (BN / WN) == 4
               ...
               : 2)) void gemm_wmma_tiled_pipeline_reuse(const half
                                                                *__restrict__ A,
                                                            const half
                                                                *__restrict__ B,
                                                            half
...
template <int BM, int BN, int BK, int WM, int WN, int STAGES>
static void launch_reuse(const half *A, const half *B, half *C, int M, int N,
  ...
  gemm_wmma_tiled_pipeline_reuse<BM, BN, BK, WM, WN, STAGES>
...
extern "C" void solve(const half *A, const half *B, half *C, int M, int N,
  ...
  if (full_aligned) {
    ...
    if (M % 128 == 0 && N % 128 == 0 && K % 32 == 0 && K >= 2048 &&
      ...
      launch_reuse<128, 128, 32, 64, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
    // The 96x128 tile uses 237 registers/thread and fits two blocks per SM
    // on A800. Small grids cannot amortize its larger per-warp work. M=192
    ...
    if (M % 192 == 0 && N % 128 == 0 && K % 32 == 0 &&
      ...
      launch_reuse<96, 128, 32, 48, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
    if (M % 64 == 0 && N % 128 == 0 && K % 32 == 0 && K >= 512 &&
        (size_t(M) / 64) * (size_t(N) / 128) >= 256) {
      launch_reuse<64, 128, 32, 32, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
    ...
    if (M % 64 == 0 && N % 64 == 0 && K % 64 == 0 && K >= 192 &&
      ...
      launch_reuse<64, 64, 64, 32, 32, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
#endif
    launch_reuse<GEMM_BM, GEMM_BN, GEMM_BK, GEMM_WM, GEMM_WN, GEMM_STAGES>(
  ...
  const unsigned blocks = unsigned(tiles < 2147483647u ? tiles : 2147483647u);
  gemm_wmma_tiled_pipeline_aligned<false, 64, 64, 32, 32, 32, 16>
