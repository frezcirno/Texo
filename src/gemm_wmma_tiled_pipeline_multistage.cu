// Standalone successor to gemm_wmma_tiled_pipeline_aligned_swizzled.cu.
...
__global__ __launch_bounds__(
    ...
    GEMM_MULTISTAGE_MIN_BLOCKS > 0
        ...
        : ((BM / WM) * (BN / WN) == 4
               ...
               : 2)) void gemm_wmma_tiled_pipeline_multistage(const half
                                                                  *__restrict__ A,
                                                              const half
                                                                  *__restrict__ B,
                                                              half
...
template <int BM, int BN, int BK, int WM, int WN, int STAGES>
static void launch_multistage(const half *A, const half *B, half *C, int M,
  ...
  gemm_wmma_tiled_pipeline_multistage<BM, BN, BK, WM, WN, STAGES>
...
// input stages. Explicit GEMM_BM/BN/BK/WM/WN/STAGES disables automatic
...
extern "C" void solve(const half *A, const half *B, half *C, int M, int N,
  ...
  if (full_aligned) {
    ...
    if (M % 128 == 0 && N % 128 == 0 && K % 32 == 0 && K >= 2048 &&
      ...
      launch_multistage<128, 128, 32, 64, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
    if (M % 64 == 0 && N % 128 == 0 && K % 32 == 0 && K >= 512 &&
        (size_t(M) / 64) * (size_t(N) / 128) >= 256) {
      launch_multistage<64, 128, 32, 32, 64, 3>(A, B, C, M, N, K, alpha, beta);
      return;
    }
#endif
    launch_multistage<GEMM_BM, GEMM_BN, GEMM_BK, GEMM_WM, GEMM_WN, GEMM_STAGES>(
