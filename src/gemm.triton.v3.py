...
Uses the autotune CUDA-Graph API available in Triton 3.1.0.
...
def _select(a, b, c, M, N, K, alpha, beta, beta_zero, alpha_one, key):
    ...
    if device not in _tuners:
        _tuners[device] = triton.autotune(
            ...
            # Triton 3.1 has no do_bench= autotune argument. This API selects
            # its CUDA-Graph benchmark, which creates a side stream internally.
            use_cuda_graph=True,
            rep=30,
        )(_gemm)
    ...
    # Tuning can use a side stream; finish producing the inputs first.
    # This happens only on a cache miss, outside warmed measurements.
