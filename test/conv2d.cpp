...
reference_conv2d(const std::vector<float> &input,
                 const std::vector<float> &kernel, int input_rows,
  ...
  std::vector<float> output(static_cast<size_t>(output_rows) * output_cols);
      ...
      for (int kernel_row = 0; kernel_row < kernel_rows; ++kernel_row) {
        for (int kernel_col = 0; kernel_col < kernel_cols; ++kernel_col) {
          const size_t input_index =
              static_cast<size_t>(row + kernel_row) * input_cols + col +
              kernel_col;
          const size_t kernel_index =
              static_cast<size_t>(kernel_row) * kernel_cols + kernel_col;
          sum += static_cast<double>(input[input_index]) * kernel[kernel_index];
        }
      }
      output[static_cast<size_t>(row) * output_cols + col] =
...
static ValidationResult validate(const std::vector<float> &actual,
  ...
  constexpr double MAX_RMSE = 1.0e-5;
...
static ValidationResult run_validation(
    ...
    const std::vector<float> &reference, int input_rows, int input_cols,
    int kernel_rows, int kernel_cols) {
  const size_t output_bytes = reference.size() * sizeof(float);

  // A NaN bit pattern catches output elements that the kernel fails to write.
  CUDA_CHECK(cudaMemset(device_output, 0xff, output_bytes));
  solve(device_input, device_kernel, device_output, input_rows, input_cols,
...
static void benchmark(const float *device_input, const float *device_kernel,
                      float *device_output, int input_rows, int input_cols,
                      int kernel_rows, int kernel_cols, int repeat,
                      int warmup) {
  ...
  const int output_rows = input_rows - kernel_rows + 1;
  const int output_cols = input_cols - kernel_cols + 1;
  const double operations = 2.0 * output_rows * output_cols * kernel_rows *
                            static_cast<double>(kernel_cols);
...
int main(int argc, char **argv) {
  ...
  if (argc > 1)
    input_rows = std::atoi(argv[1]);
  if (argc > 2)
    input_cols = std::atoi(argv[2]);
  if (argc > 3)
    kernel_rows = std::atoi(argv[3]);
  if (argc > 4)
    kernel_cols = std::atoi(argv[4]);
  if (argc > 5)
    repeat = std::atoi(argv[5]);
  if (argc > 6)
    warmup = std::atoi(argv[6]);

  if (input_rows <= 0 || input_cols <= 0 || kernel_rows <= 0 ||
      kernel_cols <= 0 || kernel_rows > input_rows ||
      kernel_cols > input_cols || repeat <= 0 || warmup < 0) {
    std::fprintf(stderr,
                 "Usage: %s [input-rows] [input-cols] [kernel-rows] "
                 "[kernel-cols] [positive-repeat] [nonnegative-warmup]\n",
                 argv[0]);
    return EXIT_FAILURE;
  }
  ...
  const size_t input_elements =
      static_cast<size_t>(input_rows) * input_cols;
  const size_t kernel_elements =
      static_cast<size_t>(kernel_rows) * kernel_cols;
  ...
  const std::vector<float> reference =
      reference_conv2d(host_input, host_kernel, input_rows, input_cols,
                       kernel_rows, kernel_cols);
  ...
  const ValidationResult validation =
      run_validation(device_input, device_kernel, device_output, reference,
                     input_rows, input_cols, kernel_rows, kernel_cols);

  cudaDeviceProp properties{};
  CUDA_CHECK(cudaGetDeviceProperties(&properties, 0));
  std::printf("CUDA conv2d benchmark\n");
  std::printf("  device             = %s\n", properties.name);
  std::printf("  input              = (%d, %d)\n", input_rows, input_cols);
  ...
  std::printf("  RMSE               = %.6e\n", validation.rmse);
  benchmark(device_input, device_kernel, device_output, input_rows, input_cols,
            kernel_rows, kernel_cols, repeat, warmup);
